...
try:
    ...
    SERVERS = [
        ...
        {"id": "S5", "name": "Server 5", "url": "https://video-editor-server-5.onrender.com"},
    ]
...
def api_call(base_url, path, method="GET"):
    url = f"{base_url}{path}"
...
class ControlPanel(tk.Tk):
    ...
    def _refresh_all(self):
        ...
        total_all = 0
        for srv in SERVERS:
            data = api_call(srv["url"], "/status")
            self._server_data[srv["id"]] = data
            card = self.cards[srv["id"]]
            d, t = card.get_done_total(data)
            total_done += d
            total_all += t
            self.after(0, card.update_status, data)

        def update_global():
            if total_all > 0:
                pct = (total_done / total_all) * 100
                self.global_progress_var.set(pct)
                self.global_label.config(text=f"{total_done}/{total_all}  ({pct:.0f}%)")
            else:
                self.global_progress_var.set(0)
                self.global_label.config(text="-- / --")

        self.after(0, update_global)
